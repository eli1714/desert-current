...
function desert_current_get_home_hero_notes( $post_id = 0 ) {
	$rows  = get_field( 'home_hero_notes', $post_id );
	$notes = array();
	...
	foreach ( $rows as $row ) {
		$label = isset( $row['note_label'] ) ? trim( (string) $row['note_label'] ) : '';
		$value = isset( $row['note_value'] ) ? trim( (string) $row['note_value'] ) : '';
